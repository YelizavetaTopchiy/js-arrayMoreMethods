...
function getMostFuelEfficientCars(cars, brand) {
  const isAllFuelEfficient = cars.every(car => car.fuelConsumption < 6);

  if (!isAllFuelEfficient) {
    return [];
  }

  const carsOfBrand = cars
    .filter(car => car.brand === brand)
    .sort((carA, carB) => carA.fuelConsumption - carB.fuelConsumption);
